 ...
 */
const defaultLogger = {
  debug: (message: string, ...args: any[]) => console.debug(`[SSE] ${message}`, ...args),
  error: (message: string, ...args: any[]) => console.error(`[SSE] ${message}`, ...args)
};
...
export function useSSE(
   ...
   */
  const parseEventData = useCallback((eventData: string): any => {
    try {
  ...
  }, [url, cleanupConnection, parseEventData, eventHandler, logger, autoReconnect, maxReconnectAttempts, initialReconnectDelay]);
